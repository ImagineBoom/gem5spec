...
# =========================================================================================================== #
# gem5_py_opts是一个数组，数组中的一项代表一次任务中gem5的参数配置。这里的一次任务，表示调用一次./run.sh。
# 默认只需要填充gem5_py_opts数组即可，然后运行此脚本。
...
for ((i=0; i<${#gem5_py_opts[@]}; i++))
do
    gem5_py_opt=${gem5_py_opts[$i]}

    echo "the $[i+1] times begin"
    echo "gem5_py_opt= '${gem5_py_opt}'"
    # label的值会影响Excel表格的命名，日期之后插入。可以自定义一个名称来区分当前这一组gem5_py_opts定义的任务。默认是数组长度。
    ./run.sh --gem5 --spec2017 --restore_all -j 24 --gem5_py_opt "${gem5_py_opt}" --label "${#gem5_py_opts[@]}"
